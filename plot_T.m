...
total_convergence_error = zeros(maxFrames, 4, length(Ts_range));
total_ase = zeros(maxFrames, 4, length(Ts_range));
mean_total_convergence_error = zeros(maxFrames, 4, length(Ts_range), maxRealizations);
mean_total_ase = zeros(maxFrames, 4, length(Ts_range), maxRealizations);

%% Monte-Carlo realizations
for r=1:maxRealizations

for ts_indx=1:length(Ts_range)
    Ts = Ts_range(ts_indx);
...
%         end
        ...
        % sub 2
        b = (vec(V1) + rho*vec(Y) + vec(Y_proposed_hbf) + vec(V2) + rho*vec(C) + rho*(gamma*vec(Yp) + sqrt(1-gamma^2)*K2*s));
...
%        Conventional HBF with ZC, which gathers Nr RF chains by assuming longer channel coherence time
        ...
        %% Adaptive OMP       %%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Yp = A*Zbar_ada_omp*B;
        Zbar_ada_omp = gamma*Zbar_ada_omp + sqrt(1-gamma^2)*reshape(z_ada_omp, Nr, Nt*L);
        ada_omp_rhs = vec(Y_hbf_nr-gamma*Yp);
        R = kron(B.', A);
        targetMatrix=[];
    ...
    total_convergence_error(:, :, ts_indx) = convergence_error;
    total_ase(:, :, ts_indx) = ase;
end
    mean_total_convergence_error(:,:,:,r) = total_convergence_error;
    mean_total_ase(:,:,:,r) = total_ase;
end

figure;
p=semilogy(Ts_range*Nt, squeeze(mean(total_convergence_error(end, 1, :), 4)));
set(p,'LineWidth',1, 'LineStyle', '--', 'Color', 'Green', 'Marker', 'o');
hold on;
p=semilogy(Ts_range*Nt, squeeze(mean(total_convergence_error(end, 2, :), 4)));
set(p,'LineWidth',1, 'LineStyle', '-', 'Color', 'Black', 'Marker', 'x');
hold on;
p=semilogy(Ts_range*Nt, squeeze(mean(total_convergence_error(end, 3, :), 4)));
set(p,'LineWidth',1, 'LineStyle', '-', 'Color', 'Black', 'Marker', 's');
hold on;
p=semilogy(Ts_range*Nt, squeeze(mean(total_convergence_error(end, 4, :), 4)));
set(p,'LineWidth',1.5, 'LineStyle', '-', 'Color', 'Black', 'Marker', 'h');
legend('Proposed tracking technique', 'AdaOMP',  'OMP',  'OMP long training', 'Location', 'Best');
grid on;
xlabel('Training length T', 'FontSize', 11)
ylabel('NMSE', 'FontSize', 11)
savefig('./results/plot_T.fig');